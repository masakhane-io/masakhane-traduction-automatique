...
def create_corpus(src, trg, en_test_sents, filedir, lc=False, seed=42, bpe_size=10000, dev_size=1000, test_dir="test"):
  """ Créer un corpus BPE pré-traité avec des répartitions aléatoires train/dev/test. """
  ...
  # effectuer le split entre dev/test/train et création de corpus parallèles
  num_dev_patterns = dev_size
  # les données de test sont chargées à partir d'un fichier
  # num_test_patterns = 1000 

  # Mettre en minuscules les corpus
  if lc:
  ...
  # Former le BPE (séparément pour src et trg)
...
if __name__ == "__main__":
  ...
  # charger d'abord les phrases de test globales pour les filtrer
  # stocker la partie anglaise dans le set (lot) pour des contrôles de filtrage rapides
  en_test_sents = set()
